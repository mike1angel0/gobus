...
export const providerSchema = z.object({
  ...
  createdAt: z.string().datetime().max(30).describe('Provider creation timestamp'),
...
export const revenueByRouteSchema = z.object({
  ...
  revenue: z.number().min(0).max(100000000).describe('Total revenue for this route'),
});

/** Dashboard analytics for a provider. */
export const providerAnalyticsSchema = z.object({
  totalBookings: z.number().int().min(0).max(2147483647).describe('Total number of confirmed bookings'),
  totalRevenue: z.number().min(0).max(100000000).describe('Total revenue from confirmed bookings'),
